...
**Annotation coordinate model**: `x`/`y` is the **bottom-left corner**; `width`/`height` extend **rightward and upward**. To enclose operators, set `x = min_op_x - 70`, `y = min_op_y - 70`, `width = (max_op_x + op_w) - min_op_x + 140`, `height = (max_op_y + op_h) - min_op_y + 240` (70 bottom + 170 top — extra top space prevents operators from overlapping the title/body text). Common mistake: setting `y` above the operators places the annotation's bottom edge above them.
