...
class EmbodyExt:
    ...
    def _enableEnvoy(self):
        """Enable Envoy: install deps, extract CLAUDE.md, start server."""
    ...
    def _extractClaudeMd(self):
        ...
        if claude_md_path.exists():
            ...
            if '<!-- Generated by Embody/Envoy' in existing:
                ...
                # User's own file -- write to ENVOY.md instead
    ...
    def _upgradeEnvoy(self):
        """Silently extract CLAUDE.md if Envoy is enabled but file is missing."""
        if not self.my.par.Envoyenable.eval():
            return
        target_dir = self._findProjectRoot()
        if not (target_dir / 'CLAUDE.md').exists() and not (target_dir / 'ENVOY.md').exists():
            self._extractClaudeMd()
