...
- **Ease every camera move**: run position/target control channels through a Filter CHOP (`type=gauss`, `width` 0.2-0.5 -- camera moves usually sit at the wider end -- with `widthunit` set to Seconds so the width is a time, not a sample count). Camera Blend COMP blends whole camera rigs for shot-to-shot moves.
