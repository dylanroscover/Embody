...
# Reconciler tick intervals.  The heartbeat cadence is dynamic:
# HEARTBEAT_TICK_S_FAST while disconnected or immediately after a state
# change, HEARTBEAT_TICK_S_STABLE once the connection has been stable
# for STABILITY_THRESHOLD_S seconds.  This keeps reconnect detection
# snappy while eliminating textport log flood once the link is healthy.
CONFIG_TICK_S = 1                 # .envoy.json mtime polling
HEARTBEAT_TICK_S_FAST = 5         # backend HTTP ping (unstable / disconnected)
HEARTBEAT_TICK_S_STABLE = 30      # backend HTTP ping (stable connection)
STABILITY_THRESHOLD_S = 30        # seconds without a state change before slowing
...
# ---------------------------------------------------------------------------
#
# The bridge's reconciler thread pings the backend every few seconds, and
# the main thread forwards client requests on top of that.  Opening a new
# socket per call via ``urllib.request.urlopen`` was leaking file
# descriptors and triggering ``starlette.requests.ClientDisconnect``
# tracebacks on the server side because urllib tears the socket down
# while Starlette is still reading the request body.
#
# We keep one persistent ``http.client.HTTPConnection`` per target URL.
# ``http.client`` connections are NOT thread-safe, so every request holds
# ``_http_pool_lock`` for the full request/response lifecycle.  That is
# fine: the bridge is single-user and the request rate is low.

_http_pool_lock = threading.Lock()
_http_pool = {}  # url (str) -> http.client.HTTPConnection


def _get_http_connection(url):
    """Return the pooled connection for ``url``, creating it on first use.

    Caller MUST hold ``_http_pool_lock``.
    """
    conn = _http_pool.get(url)
    if conn is not None:
        return conn
    parsed = urlparse(url)
    ...
    if parsed.scheme == "https":
        conn = http.client.HTTPSConnection(host, port, timeout=BACKEND_PING_TIMEOUT_S)
    else:
        conn = http.client.HTTPConnection(host, port, timeout=BACKEND_PING_TIMEOUT_S)
    _http_pool[url] = conn
    return conn


def _close_http_connection(url):
    """Pop and close the pooled connection for ``url``.  Best-effort.

    Caller MUST hold ``_http_pool_lock``.
    """
    conn = _http_pool.pop(url, None)
    if conn is None:
        return
    try:
        conn.close()
...
class BridgeState:
    ...
    def __init__(self, *, url, td_pid=None, config=None,
        ...
        # Timestamp (time.time()) of the last connect/disconnect/URL-switch.
        # Used by the reconciler's dynamic heartbeat cadence: stay on the
        # fast cadence until the link has been stable for STABILITY_THRESHOLD_S
        # seconds, then slow down to HEARTBEAT_TICK_S_STABLE.
...
def handle_switch_instance(params, state):
    ...
    # Release any pooled connection to the old URL.  Done outside the
    # state lock to avoid nested lock acquisition; _close_http_connection
    # requires _http_pool_lock.
...
def reconcile(state, on_tools_change, *, heartbeat):
    ...
    if new_url is not None:
        with state:
            ...
            state.cached_tools_hash = None
            state.last_state_change_time = time.time()
        # Release the stale pooled connection for the old URL without
        # holding the state lock — ``_close_http_connection`` acquires
        # its own lock (``_http_pool_lock``) internally.
        with _http_pool_lock:
            _close_http_connection(old_url)
        url_switched = True
    ...
    with state:
        ...
        if is_up:
            ...
            state.last_connected_time = time.time()
        if became_connected or became_disconnected:
            state.last_state_change_time = time.time()
...
def _current_heartbeat_interval_s(state):
    """Return the current heartbeat interval based on connection stability.

    While disconnected (or immediately after a connect/disconnect/URL
    switch) we stay on the fast cadence so reconnect detection is
    snappy.  Once the link has been stable for ``STABILITY_THRESHOLD_S``
    seconds we slow the ping cadence to ``HEARTBEAT_TICK_S_STABLE`` to
    stop flooding TouchDesigner's textport with log lines from the
    per-request FastMCP handler.
    """
    with state:
        if not state.connected:
            return HEARTBEAT_TICK_S_FAST
        last_change = state.last_state_change_time
    if last_change == 0:
        return HEARTBEAT_TICK_S_FAST
    if time.time() - last_change >= STABILITY_THRESHOLD_S:
        return HEARTBEAT_TICK_S_STABLE
    return HEARTBEAT_TICK_S_FAST
...
def start_reconciler(state, on_tools_change):
    ...
    the cheap config-mtime check every tick (Phase 1) and the more
    expensive backend heartbeat on a dynamic cadence (Phase 2) —
    ``HEARTBEAT_TICK_S_FAST`` while disconnected or unstable,
    ``HEARTBEAT_TICK_S_STABLE`` after the link has been stable for
    ``STABILITY_THRESHOLD_S``.  All exceptions inside the loop body are
    caught and logged — the thread must never die.
    """
    def loop():
