...
class TDNExt:
	...
	def _loadDivergentDefaults(self):
		...
		try:
			catalog_path = self.ownerComp.ext.CatalogManager._getCatalogPath(build_str)
			if os.path.isfile(catalog_path):
				catalog = self.ownerComp.ext.CatalogManager._readCatalog(catalog_path)
				if catalog:
	...
	def _restoreTOXShells(self, dest) -> int:
		...
		def _walk(comp):
			...
			for child in list(getattr(comp, 'children', ()) or ()):
				...
				if pending:
					try:
						normalized = self.ownerComp.ext.Embody.normalizePath(pending)
						child.par.externaltox = normalized
						child.par.enableexternaltox = True
						self.ownerComp.ext.Embody._reloadTox(child)
