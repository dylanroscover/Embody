...
def _make_valid_tdxn(op_count=1):
...
def _make_valid_tdxn_json(op_count=1):
	"""Build a minimal valid TDXN JSON string."""
	return json.dumps(_make_valid_tdxn(op_count))
...
class TestTDXNCrashSafety(EmbodyTestCase):
	...
	def setUp(self):
		...
		self._tdxn_dir = os.path.join(self._temp_dir, 'embody', 'Foo')
		os.makedirs(self._tdxn_dir, exist_ok=True)
		self._tdxn_path = os.path.join(self._tdxn_dir, 'bar.tdn')
	...
	def test_A01_atomic_write_creates_file(self):
		...
		self.tdn._atomic_write(self._tdxn_path, content)
		self.assertTrue(Path(self._tdxn_path).is_file())
		self.assertEqual(Path(self._tdxn_path).read_text(encoding='utf-8'), content)

	def test_A02_atomic_write_replaces_existing(self):
		"""Atomic write should replace existing file content."""
		Path(self._tdxn_path).write_text('old content', encoding='utf-8')
		new_content = _make_valid_tdxn_json(5)
		self.tdn._atomic_write(self._tdxn_path, new_content)
		self.assertEqual(
			Path(self._tdxn_path).read_text(encoding='utf-8'), new_content)

	def test_A03_atomic_write_no_temp_residue(self):
		"""No .tmp files should remain after successful atomic write."""
		self.tdn._atomic_write(self._tdxn_path, _make_valid_tdxn_json())
		tmp_files = list(Path(self._tdxn_dir).glob('*.tmp'))
	...
	def test_A04_atomic_write_readonly_dir_fails_cleanly(self):
		...
		raised = False
		try:
			self.tdn._atomic_write(target, _make_valid_tdxn_json())
		except Exception:
			raised = True
	...
	def test_B00b_find_falls_back_to_legacy_tdxn_backup(self):
		...
		legacy = self.tdn._get_backup_path(
			self._tdxn_path, self._proj_folder, '.bak', legacy=True)
		legacy.parent.mkdir(parents=True, exist_ok=True)
		legacy.write_text(_make_valid_tdxn_json(7), encoding='utf-8')

		found = self.tdn._find_existing_backup(
			self._tdxn_path, self._proj_folder)
		self.assertIsNotNone(found, 'legacy backup must be found')
		self.assertEqual(found.read_text(encoding='utf-8'),
						 _make_valid_tdxn_json(7))
	...
	def test_B00c_newest_wins_regardless_of_directory(self):
		...
		cur.write_text(_make_valid_tdxn_json(2), encoding='utf-8')
		os.utime(cur, (1_000_000, 1_000_000))          # old

		legacy = self.tdn._get_backup_path(
			self._tdxn_path, self._proj_folder, '.bak', legacy=True)
		legacy.parent.mkdir(parents=True, exist_ok=True)
		legacy.write_text(_make_valid_tdxn_json(9), encoding='utf-8')
		...
		self.assertEqual(
			self.tdn._find_existing_backup(
				self._tdxn_path, self._proj_folder), cur)
	...
	def test_B00d_corrupt_bak_falls_through_to_bak2(self):
		...
		Path(self._tdxn_path).write_text(
			_make_valid_tdxn_json(3), encoding='utf-8')
		self.tdn._rotate_backups(self._tdxn_path, self._proj_folder)
		Path(self._tdxn_path).write_text(
			_make_valid_tdxn_json(4), encoding='utf-8')
		self.tdn._rotate_backups(self._tdxn_path, self._proj_folder)
	...
	def test_B00d2_migrated_suffix_backups_are_still_found(self):
		...
		migrated = os.path.join(self._tdxn_dir, 'bar.tdxn')
		old_name_bak = self.tdn._get_backup_path(
			self._tdxn_path, self._proj_folder, '.bak')   # bar.tdn.bak
		old_name_bak.parent.mkdir(parents=True, exist_ok=True)
		old_name_bak.write_text(_make_valid_tdxn_json(5), encoding='utf-8')
	...
	def test_B00d3_rotation_retires_this_files_legacy_copies(self):
		...
		legacy = self.tdn._get_backup_path(
			self._tdxn_path, self._proj_folder, '.bak', legacy=True)
		legacy.parent.mkdir(parents=True, exist_ok=True)
		legacy.write_text(_make_valid_tdxn_json(1), encoding='utf-8')
		legacy2 = self.tdn._get_backup_path(
			self._tdxn_path, self._proj_folder, '.bak2', legacy=True)
		legacy2.write_text(_make_valid_tdxn_json(1), encoding='utf-8')

		Path(self._tdxn_path).write_text(
			_make_valid_tdxn_json(6), encoding='utf-8')
		self.tdn._rotate_backups(self._tdxn_path, self._proj_folder)
		...
		self.assertTrue(
			self.tdn._get_backup_path(
				self._tdxn_path, self._proj_folder, '.bak').is_file(),
	...
	def test_B00f_rotation_failure_rides_back_on_the_result(self):
		...
		"""
		Path(self._tdxn_path).write_text('old', encoding='utf-8')
		...
		self.assertEqual(
			Path(self._tdxn_path).read_text(encoding='utf-8'),
			_make_valid_tdxn_json(),
	...
	def test_B02_rotate_second_export_rotates(self):
		...
		Path(self._tdxn_path).write_text('v2', encoding='utf-8')
		self.tdn._rotate_backups(self._tdxn_path, self._proj_folder)
		bak = self.tdn._get_backup_path(
			self._tdxn_path, self._proj_folder, '.bak')
		bak2 = self.tdn._get_backup_path(
	...
	def test_B03_rotate_third_export_overwrites_bak2(self):
		...
		Path(self._tdxn_path).write_text('v2', encoding='utf-8')
		self.tdn._rotate_backups(self._tdxn_path, self._proj_folder)
		Path(self._tdxn_path).write_text('v3', encoding='utf-8')
		self.tdn._rotate_backups(self._tdxn_path, self._proj_folder)
		bak = self.tdn._get_backup_path(
			self._tdxn_path, self._proj_folder, '.bak')
		bak2 = self.tdn._get_backup_path(
	...
	def test_B04_rotate_noop_when_no_file(self):
		...
		nonexistent = os.path.join(self._tdxn_dir, 'nope.tdn')
	...
	def test_B06_rotate_creates_parent_dirs(self):
		...
		deep_tdxn = os.path.join(deep_dir, 'test.tdn')
		Path(deep_tdxn).write_text('content', encoding='utf-8')
		self.tdn._rotate_backups(deep_tdxn, self._proj_folder)
		bak = self.tdn._get_backup_path(
			deep_tdxn, self._proj_folder, '.bak')
	...
	def test_B07_rotate_preserves_file_content(self):
		...
		content = _make_valid_tdxn_json(50)
		Path(self._tdxn_path).write_text(content, encoding='utf-8')
		self.tdn._rotate_backups(self._tdxn_path, self._proj_folder)
		bak = self.tdn._get_backup_path(
			self._tdxn_path, self._proj_folder, '.bak')
	...
	def test_C01_validate_valid_tdxn(self):
	...
	def test_D01_safe_write_full_cycle(self):
		...
		self.assertEqual(
			Path(self._tdxn_path).read_text(encoding='utf-8'), v2)
	...
	def test_D03_safe_write_restores_on_validation_failure(self):
		...
		restored = Path(self._tdxn_path).read_text(encoding='utf-8')
	...
	def test_E05_missing_tdxn_file_with_backup(self):
		"""When .tdn is deleted but .bak exists, backup is usable."""
		v1 = _make_valid_tdxn_json(3)
		Path(self._tdxn_path).write_text(v1, encoding='utf-8')
		self.tdn._rotate_backups(self._tdxn_path, self._proj_folder)
		os.unlink(self._tdxn_path)
		bak = self.tdn._get_backup_path(
			self._tdxn_path, self._proj_folder, '.bak')
		self.assertTrue(bak.is_file())
	...
	def test_E08_disk_full_simulation(self):
		...
		v2 = _make_valid_tdxn_json(3)
		self.tdn._atomic_write(self._tdxn_path, v2)
		self.assertEqual(
			Path(self._tdxn_path).read_text(encoding='utf-8'), v2)
	...
	def test_F01_1000_operator_export_roundtrip(self):
		...
		# Export to temp file
		tdxn_file = os.path.join(self._temp_dir, 'stress.tdn')
		...
		imp = self.tdn.ImportNetwork(
			target_path=self.sandbox.path, tdn=tdxn_doc, clear_first=True)
	...
	def test_F02_1000_operator_backup_rotation(self):
		...
		v1_content = Path(tdxn_file).read_text(encoding='utf-8')
		# Modify a parameter
	...
	def test_G01_skips_rewrite_when_network_is_unchanged(self):
		...
		before = open(self._tdxn_path, 'rb').read()
		mtime = os.path.getmtime(self._tdxn_path)
		...
		self.assertEqual(open(self._tdxn_path, 'rb').read(), before,
						 'file was rewritten despite an identical network')
		self.assertEqual(os.path.getmtime(self._tdxn_path), mtime,
	...
	def test_G02_still_writes_a_real_content_change(self):
		...
		before = open(self._tdxn_path, 'rb').read()
		res = self._write(_make_valid_tdxn(3))
		self.assertTrue(res.get('success'))
		self.assertFalse(res.get('skipped'), 'a real change must be written')
		self.assertNotEqual(open(self._tdxn_path, 'rb').read(), before)
	...
	def test_G03_still_writes_the_format_convergence(self):
		...
		before = open(self._tdxn_path, 'rb').read()
		res = self._write(dict(_make_valid_tdxn(1), format='tdxn'))
		self.assertTrue(res.get('success'))
	...
	def test_G04_skip_does_not_rotate_backups(self):
		...
		res = self._write(_make_valid_tdxn(2))          # identical -> skip
		self.assertTrue(res.get('skipped'))
	...
	def test_F03_1000_operator_corrupt_and_rollback(self):
		...
		Path(tdxn_file).write_text(_make_valid_tdxn_json(1), encoding='utf-8')
		...
		for c in list(self.sandbox.children):
			c.destroy()
		imp = self.tdn.ImportNetwork(
			target_path=self.sandbox.path, tdn=valid_tdxn, clear_first=True)
	...
	def test_F04_1000_operator_partial_import_recovery(self):
		...
		self.assertTrue(result.get('success'))
		full_tdxn = result['tdn']
		full_count = len(full_tdxn['operators'])
		# Make a partial copy with only half the operators
		import copy
		partial_tdxn = copy.deepcopy(full_tdxn)
		partial_tdxn['operators'] = full_tdxn['operators'][:full_count // 2]
		...
		imp_partial = self.tdn.ImportNetwork(
			target_path=self.sandbox.path, tdn=partial_tdxn, clear_first=True)
		...
		imp_full = self.tdn.ImportNetwork(
			target_path=self.sandbox.path, tdn=full_tdxn, clear_first=True)
	...
	def test_F07_mixed_failure_modes(self):
		...
		# Corrupt .tdn and .bak
		Path(tdxn_file).write_text('CORRUPT', encoding='utf-8')
		bak = self.tdn._get_backup_path(
			tdxn_file, self._proj_folder, '.bak')
		self.assertTrue(bak.is_file(), '.bak should exist')
		...
		imp = self.tdn.ImportNetwork(
			...
			tdn=recovered_tdxn, clear_first=True)
		self.assertTrue(imp.get('success'),
