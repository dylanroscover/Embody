...
class TestMCPConnections(EmbodyTestCase):
    ...
    def test_connect_tops(self):
        ...
        result = self.embody.ext.Envoy._connect_ops(
    ...
    def test_disconnect_after_connect(self):
        ...
        self.embody.ext.Envoy._connect_ops(
            source_path=noise.path, dest_path=level.path)
        result = self.embody.ext.Envoy._disconnect_op(op_path=level.path)
        self.assertTrue(result.get('success'))

    def test_disconnect_nonexistent(self):
        result = self.embody.ext.Envoy._disconnect_op(op_path='/nonexistent')
        self.assertDictHasKey(result, 'error')
    ...
    def test_get_connections_basic(self):
        ...
        self.embody.ext.Envoy._connect_ops(
            source_path=noise.path, dest_path=level.path)
        result = self.embody.ext.Envoy._get_connections(op_path=level.path)
        self.assertDictHasKey(result, 'inputs')

    def test_get_connections_empty(self):
        comp = self.sandbox.create(baseCOMP, 'no_conn')
        result = self.embody.ext.Envoy._get_connections(op_path=comp.path)
        self.assertDictHasKey(result, 'inputs')

    def test_get_connections_nonexistent(self):
        result = self.embody.ext.Envoy._get_connections(op_path='/nonexistent')
        self.assertDictHasKey(result, 'error')
