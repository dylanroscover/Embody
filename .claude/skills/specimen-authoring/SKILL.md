...
### CRITICAL: TDN does NOT round-trip custom-parameter VALUES
A specimen's exposed parameters (bound to GLSL uniforms) export their **definitions** (default, range, help) but **not their current value** when the value equals the default. On `import_network`/`ImportNetworkFromFile` the params are recreated with the right `default` but `.val` stays at 0/min - so **every parametric specimen imports inert** (Cohesion=0, Separation=0, ...) until something sets `par.val = par.default`. Verify this in every re-import check. Until the TDN importer is fixed to apply default->value, a freshly-imported parametric specimen needs a one-time "revert to default" (or an `onInitTD` that sets values). This affects the whole collection (noise-terrain has 10 such params).
