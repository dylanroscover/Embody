...
5. **Bake the RESULT thumbnail.** `build_specimens.py` does not exist yet, so bake by hand -- this is required, not optional; an unbaked specimen shows "preview coming soon" in the gallery:
   ...
   c. Save to `platform/apps/web/public/specimens/<slug>.jpg` (slug kebab-case), and set the manifest's `thumbnail_path` to `/specimens/<slug>.jpg`.
   d. Register the slug in the `BAKED_THUMBS` set in `platform/apps/web/src/components/SpecimenCover.astro` -- that is what makes the gallery's RESULT view load the render instead of the placeholder SVG (the gallery reads the component set, not `thumbnail_path`, until the data flow is unified).
