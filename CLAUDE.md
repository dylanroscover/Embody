...
1. **Prefer `.tdn` files for reading TDN-externalized COMPs** — `.tdn` files are JSON on disk with complete network structure (operators, parameters, connections, positions, flags, DAT content, annotations). Reading them directly is faster than MCP round-trips. Check `externalizations.tsv` (strategy column) or call `get_externalizations` to identify TDN-strategy COMPs. To edit: modify the `.tdn` file on disk, then **always** call `import_network` via MCP with the COMP path, the parsed JSON, and `clear_first=True` to reload it in TD. **Never leave a `.tdn` edit unreloaded** — the user must see updates immediately in TD. Use MCP when you need live runtime state (evaluated expressions, cook errors) or for non-TDN operators. **To answer "what's changed / what's unsaved?" for TouchDesigner networks, call the `diff_tdn` MCP tool** — it shows what is UNSAVED (the live in-memory network vs the on-disk `.tdn`), which git cannot see (git only reads disk, never TD's live state). With **no `target`** (or `target=""`) it diffs the **whole project** — every live TDN COMP, summarized (which changed + counts); use this for "what's changed across the project / what haven't I saved." With a **`target`** (a COMP path **or** a `.tdn` file path/bare filename like `"tooltip.tdn"`, resolved to its COMP) it diffs that one COMP in full per-field detail (`old`=disk, `new`=live). It normalizes away export-header churn, reporting only real network changes (operators, parameters, connections, annotations) matched by name per level. For **committed / history** diffs (what changed vs the last commit), use plain `git diff` / `git log` — Embody installs a `.tdn` git diff driver so those are clean too (the volatile header is stripped; a no-op re-export shows nothing). Read-only, non-interactive, never auto-run. Don't hand-diff `.tdn` files.
