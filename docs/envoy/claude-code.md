...
|---|---|---|
| `AGENTS.md` | Universal AI instructions (always written, whichever clients are selected) | Yes |
| `CLAUDE.md` | Project context and critical rules | Yes -- or merged into, if the file is yours (see below) |
...
Embody **merges** into it rather than replacing or skipping it. Your content is
left exactly as written, and Embody maintains one delimited block inside the
file -- opened by a visible `## Embody / Envoy -- auto-generated section`
heading that says which half is which and where your own edits belong. Later
deploys refresh that block in place, wherever you moved it. Uninstall removes
only the block and leaves the rest of your file.

Earlier versions instead wrote the guidance to a separate `ENVOY.md`. That
preserved your file but delivered the instructions nowhere, because Claude Code
does not auto-load `ENVOY.md`. Embody no longer writes that file; an `ENVOY.md`
left over from an older version is not touched, and you can delete it once the
block is present in your `CLAUDE.md`.

The `Yes (unless edited)` files are refreshed from their templates on start **only while pristine**. Embody stamps each file it generates with a hash of its own content, in the `<!-- Generated by Embody/Envoy ... sha:... -->` marker line; once you edit a generated rule or skill the hash no longer matches, your version is preserved, and Embody says so in the log — delete the file to opt back into regeneration. Because the hash rides inside the file, that protection survives a clone: a teammate who pulls your edited rule keeps it too. See [How It Works](#how-it-works).
...
- **Templates are the source of truth for pristine files** — an unedited generated `.claude/` file is refreshed from its template on Envoy start, but Embody stamps each file it generates with a hash of its own content, in the `<!-- Generated by Embody/Envoy ... sha:... -->` marker line and will not overwrite one you've since edited (delete the file to opt back into regeneration). The stamp travels with the file, so the protection holds on a clone, a fresh checkout, or any machine that has never run Embody
...
!!! tip
    You can edit the Envoy-generated rules and skills directly — Embody stamps each file it generates with a hash of its own content, in the `<!-- Generated by Embody/Envoy ... sha:... -->` marker line, so it won't overwrite one you've changed (it logs that it kept your edits). Commit your edited file and it stays protected everywhere: the hash is inside the file, not in a machine-local sidecar. Pristine generated files still refresh on regeneration; delete a file to discard your changes and pull the latest template.
