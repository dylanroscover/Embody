...
`.embody/project.json` remains **committed** (`.embody/*` + `!.embody/project.json` in `.gitignore`) as the home for project-level metadata that genuinely travels with the repo. Embody stewards it with key-level ownership: it only ever touches keys it owns, preserves everything else, and never overwrites a file it cannot parse. Today it carries the Convoy identity (`convoy`), any declared Python extras (`python.extras`), and Envoy's project declaration (`"envoy": {"enabled": true}`) — written once while Envoy is on, and read only by a machine that has no `.embody/config.json` of its own, so a fresh clone turns Envoy on at first open instead of arriving with no `.mcp.json`. A local `config.json` always wins, including a deliberate off. A repo can opt out by ignoring the file itself: Envoy checks what git actually does (`git check-ignore`) rather than pattern-matching, and when the repo's own rules ignore the file it stops adding `!.embody/project.json` and logs a warning — the metadata is then per-machine instead of shared through git.
