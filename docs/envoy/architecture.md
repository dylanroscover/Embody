...
The bridge reads project configuration from `.envoy.json` at the git root:

| Field | Description |
|-------|-------------|
| `toe_path` | Relative path to the `.toe` project file (e.g., `dev/Embody-5.toe`) |
| `port` | Envoy's HTTP port (default: 9870) |
| `td_executable` | Path to the TouchDesigner executable or `.app` bundle |

This file is auto-generated by Envoy on startup and added to `.gitignore`.
