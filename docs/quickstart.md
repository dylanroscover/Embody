...
- **TouchDesigner 2025.33070 or later** — Windows or macOS. [Download from Derivative](https://derivative.ca/download).
- **An AI assistant that speaks [MCP](https://modelcontextprotocol.io/)** (the open standard that lets AI tools drive other apps) — [Claude Code](https://docs.anthropic.com/en/docs/claude-code) (recommended), [OpenCode](https://opencode.ai/), [Codex](https://github.com/openai/codex), [Gemini](https://github.com/google-gemini/gemini-cli), [Cursor](https://www.cursor.com/), [Windsurf](https://windsurf.com/), or GitHub Copilot via VS Code.
...
The easiest way in is the **Launch AI Client** button: on the Embody component's **Envoy** parameter page, set the **Launch Client** menu to your assistant, then click **Launch AI Client**. Embody opens the client (an editor as a workspace, or a CLI in a new terminal) already pointed at your project root — and if the client isn't installed yet, the terminal walks you through it: the official install command for your OS on its own line, ready to copy/paste.
