...
### Faithful status for ALL externalized files

- **Feature: git-uncommitted indicator (orange badge).** Embody already showed an "unsaved" (red) badge for TOX/TDN COMPs -- the live network differing from the on-disk file, which git cannot see. But externalized DAT scripts use TouchDesigner's bidirectional `syncfile`, so their live text and disk file are always identical -- they are never "unsaved," and previously showed no change state at all. Their only meaningful "changed" axis is git-relative: on disk but not committed. Embody now computes this on each refresh -- the `git status --porcelain` subprocess runs on a **worker thread** (off the main thread, so it never stalls a frame), folder-scoped and timeout-guarded; the cheap, string-based result-to-operator mapping and the store happen back on the main thread and then update the badges -- and shows a distinct **orange** badge for any externalized file that is saved-but-uncommitted -- TOX, TDN, and DAT alike. The two axes stay visually separate: **red = unsaved** (not externalized yet), **orange = saved but not committed** -- so the green/blue saved colors now mean clean AND committed. Runtime-only (stored, never written to `externalizations.tsv`, which would churn) and self-disabling outside a git repo, so non-git users see zero behavior change.
- **Feature: `changed` filter keyword.** Type `changed` in the manager's filter field to show only rows with pending changes on either axis -- unsaved (red/amber) or uncommitted (orange) -- with their ancestors, so the tree stays intact.
- **Consistency: lowercase Strategy labels.** `tox`, `tdn`, `tox par`, `tdn par`, `tag` -- matching the already-lowercase `py`/`json`/`md` file labels.
- **Rule: refresh Embody after committing.** A new shipped rule (`.claude/rules/refresh-after-commit.md`) has AI agents trigger an Embody refresh after any commit that includes externalized files, so the orange -> clean transition shows immediately without a manual refresh.
...
- **Feature: `diff_tdn` MCP tool -- the UNSAVED view git can't give.** It compares the live in-memory network against its on-disk `.tdn`, answering "what have I changed but not saved?" -- something git fundamentally cannot see (git only reads files on disk, never TD's live state). `target` accepts a COMP path **or** a `.tdn` file path/bare filename (e.g. `"tooltip.tdn"`, resolved to its COMP) for one COMP in full per-field detail; **omit `target` for a whole-project summary** across every live TDN COMP (which changed + counts). The comparison is **semantic, not byte-level**: both sides are normalized through the same `type_defaults` / `par_templates` expansion the format uses (compression-only differences read as no-change) and the volatile export header (`build`, `generator`, `td_build`, `exported_at`, `source_file`) is ignored. Operators match by name per level -- a reorder is clean, a deep child edit marks only that child. Each change is `{old, new}` with **`old`=disk, `new`=live**, tagged `kind: root | op | annotation`. Honest truncation (`counts` always true; list capped by `max_changed_ops`; per-field body-strip at `max_bytes`). The engine lives in `TDNExt` (`DiffLiveVsDisk` / `DiffAllLiveVsDisk` + pure `_diff_normalized`), reusing the existing expanders with zero duplication; `EnvoyExt._diff_tdn` is a thin main-thread delegate. A 5-reviewer integration panel caught and fixed two real bugs pre-merge (old/new were reversed against a disk baseline; the truncation cap counted only `modified`).
- **Companion: a `.tdn` git diff driver for committed / history diffs.** A raw `git diff` of a `.tdn` is buried in export-header churn -- a re-export bumps the timestamp/build even when nothing changed. Embody now installs a git **textconv** driver (`.gitattributes` `*.tdn diff=tdn`, a pure-stdlib `.embody/tdn_textconv.py`, and `git config diff.tdn.textconv`, auto-configured on Envoy startup the same way `.gitignore`/`.gitattributes`/`.mcp.json` are) that strips the volatile header before diffing -- so `git diff` / `git log -p` / `git show` on a `.tdn` show only real network changes, and a no-op re-export shows nothing. The two paths are complementary: `diff_tdn` for the unsaved window (git can't see it), the driver for the committed/on-disk view (git's domain) -- both clean. Shipped to user projects via the new `text_tdn_textconv` template DAT.
- **Discoverability: "what changed in `comp.tdn`?" routes to `diff_tdn`.** `get_externalizations` / `get_externalization_status` report each COMP's `strategy`, `absolute_path`, and a `recommended_tool: diff_tdn` hint; the MCP tool reference, root `CLAUDE.md`, the shipped `text_claude.md`, and the TDN docs describe when to reach for `diff_tdn` (unsaved) versus `git diff` (committed, kept clean by the driver). Validated with a hint-free agent that called `diff_tdn` for both single-COMP and project-wide "what changed" questions, and used `git diff` for neither.

### No cached extension references (hard rule)

- **Project-wide rule: never assign a TD extension object to a variable -- reference it inline every time.** TD reinitializes an extension whenever its externalized `.py` changes on disk or on reload; any cached reference (`emb = op.Embody.ext.Embody`, `self.envoy = ...ext.Envoy`, or even a bound method like `strip = ...ext.TDN._stripBuildSuffix`) then points at a dead instance and silently misbehaves. The entire codebase -- production and tests -- was swept to inline these at the point of use. Assigning a call's *return value*, a DAT/op the extension exposes, or the COMP itself is still fine; only the extension object and its bound methods may not be stored.
- **Enforced by `test_no_ext_caching.py`.** A new guard scans all production and test source and fails on any new cached-extension assignment. The two legitimate exceptions -- a main-thread to worker-thread handoff, and a test monkeypatch save/restore -- are opted out with a trailing `# ext-cache-ok` marker. The rule is documented in `.claude/rules/embody-code-conventions.md`.

### Envoy resilience -- the "not responding" fix

- **Fix: the recurring "TouchDesigner is not responding but the process is still running" error during saves.** Root cause: the STDIO bridge forwarded each request to TD exactly once and surfaced any transient failure immediately as "not responding" -- but during a `project.save()` the main thread (and the GIL) is busy long enough that a forward can briefly fail while TD is perfectly alive. The bridge now wraps forwarding in `forward_with_busy_retry`: on a connection-level failure it retries for up to 30 seconds **as long as the TD process is alive**, and only surfaces "not responding" if the retry window expires; if the PID is actually dead it fails fast as before. Applied to both `dev/embody/envoy_bridge.py` and the shipped `text_envoy_bridge` template, and verified auto-deployed into `.embody/envoy-bridge.py` and baked into the release `.tox`.

### Docs site

- **Fix: navigation pills (`for-ai`, `embody`, and siblings) are now readable over page content.** The marked nav links use a frosted-glass treatment -- a translucent dark background with a real `backdrop-filter` blur and isolation, plus an opaque `@supports` fallback for browsers without backdrop-filter -- so the label stays legible when it overlaps body text instead of letting the content bleed through.

### Tests & maintenance

- **Test: 60 suites / 1,449 tests** (+3 suites, +36 tests): `test_tdn_diff_engine.py` (23 -- pure normalize/diff engine), `test_tdn_diff.py` (11 -- the `DiffLiveVsDisk` handler, project-wide `DiffAllLiveVsDisk`, `.tdn`-filename resolution, and discoverability hints end-to-end), and `test_no_ext_caching.py` (2 -- the enforcement guard over production and test source). All green.
- **Fix: `dev/release_testing/smoke_bootstrap.py`** called `embody.scriptErrors` as a property; it is a method, so the ready-flag now reports `str(embody.scriptErrors())`. The release `.tox` was verified by a fresh-install smoke test (clean load into `/Embody`, zero script errors, all extensions, `diff_tdn` registered, the bridge fix present in the shipped template).
