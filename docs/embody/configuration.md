...
- **Clipboard Auto-Paste** — When ON (default), Embody watches the OS clipboard (~1.5s poll) for an **inbound** TDXN network — e.g. from the web "embody it" button — and prompts to paste it as a new COMP in the current network. Your own **outbound** copies (++ctrl+shift+c++ on a COMP, which exports it to the clipboard to share or paste elsewhere) are recognized and do **not** re-trigger the prompt. Disabled in Perform Mode and self-suppressed during saves and tests. Turn OFF to ignore the clipboard. See [Keyboard Shortcuts](keyboard-shortcuts.md).
...
- **Configure For** — Which AI client Embody generates config files for: that client's instruction files (rules, skills) and the MCP config **it** reads. `AGENTS.md` is written for every project regardless. The root `.mcp.json` — Claude Code's format, which no other client reads — is Envoy's baseline and is written whenever *either* this menu or **Launch Client** names a client; it is skipped only when both are `None`. Generation is **additive and never removes**, so selecting a second client later leaves the first one configured and still tracking bridge and port changes: pick each in turn to serve several clients from one project. Set to `None` to generate no client config at all (the Convoy-only posture). See [Which files each client gets](../envoy/setup.md#which-files-each-client-gets).
- **Launch Client** — Which client the **Launch AI Client** button opens. This opens an app and writes nothing; **Configure For** decides what lands in your repo. They are separate because they genuinely differ — configuring Cursor while launching a terminal for Claude Code is a normal setup. GitHub Copilot is an extension inside VS Code rather than an app of its own, so choosing it opens VS Code.
- **Launch AI Client** — Pulse to open the assistant selected in **Launch Client** at the project root (`_findProjectRoot()`, honoring **AI Project Root**). Editors (VS Code, Cursor, Windsurf, Antigravity; GitHub Copilot opens VS Code) open the root folder as a workspace; terminal CLIs (Claude Code, OpenCode, Codex, Gemini) open in a new terminal there. Fire-and-forget — it opens a window and does not confirm the tool actually ran. If a CLI isn't installed yet, the opened terminal shows step-by-step install instructions with the official install command for your OS on its own line, ready to copy/paste (plus an alternative installer and the docs link); install it, close the window, and press **Launch AI Client** again.
- **AI Project Root** — Where Embody writes AI/MCP config files (`AGENTS.md`, `.mcp.json`, `.embody/`, plus the per-client files: `CLAUDE.md` + `.claude/`, `opencode.json`, `.codex/config.toml`, `GEMINI.md` + `.gemini/settings.json`, `.vscode/mcp.json`, `.github/copilot-instructions.md` + `.github/instructions/`, `.cursor/`, `.windsurf/rules/`, `.agents/` — which of these appear depends on **Configure For**). Three modes:
...
- **Envoy Follow** — When ON (default), the network editor follows Envoy's work as Claude builds — gliding to center on each operator just touched, and navigating into whatever COMP is being built when the work moves to a network you're not viewing. Works with or without the Embot character shown. With Embot on, the camera settles **with him** rather than cutting to every network the work merely passes through; with Embot off it follows every hop as before. Yields the instant you pan, zoom, or navigate the view yourself, and resumes once you stop. View-only (writes pane state, never externalized). See [Claude Code Integration — Live Build Visualization](../envoy/claude-code.md#live-build-visualization).
- **Auto-Externalize New Ops** — **Opt-in**, default `Neither` (off). When set to `DATs`, `COMPs`, or `DATs and COMPs`, operators the AI creates through Envoy's `create_op` are automatically tagged and externalized to version-control-friendly files (COMPs as `.tdxn`, DATs as source files). With the default `Neither`, nothing the AI builds is written to disk unless you tag it yourself. Boundary-scoped and additive only — it never tags ops inside an already-externalized COMP, and never deletes files or removes tags.
...
- **TDXN Create on Start** — Reconstruct TDXN-strategy COMPs from `.tdxn` files on project open. **Only active when `TDXN Mode` = Roundtrip.** In Export-on-Save mode, the `.toe` is the source of truth and reconstruction is skipped. See [TDXN](#tdxn) for the full parameter listing.

### TDXN

- **TDXN Mode** — Master three-way switch for the TDXN subsystem:
    - *Off* — no TDXN runtime. `.tdxn` files on disk are preserved; Embody stops touching them.
    - *Export-on-Save* — **default**. Writes `.tdxn` files on save. `.toe` is the source of truth; live network is never stripped. Ideal for git-diff and MCP workflows.
    - *Roundtrip (Experimental)* — bidirectional strip/restore. Children are stripped from the `.toe` on save and rebuilt from `.tdxn` on open. May hit edge cases with extension reload timing on deeply-nested TDXN COMPs.
...
- **Embed DATs** — Also copy file-backed DAT content into TDXN exports; unexternalized, editable DAT content is always included
...
- **Content Safety** — What a save does about TDXN content its `.tdxn` cannot hold. *Warn Each Save* (default) logs a WARNING when a save or the next open would lose storage (Embed Storage off); *Always Externalize* also files unexternalized, editable DATs as their own files after the save (COMPs with Embed DATs off); *Never Warn* is silent. Saves never show a dialog, and unexternalized DAT content is always written into the `.tdxn`. See [Content Safety](externalization.md#content-safety-save-time-check)
- **Export Project TDXN** — Pulse to export the entire project network
- **Auto-Save Checkpoints** — When ON (default), a beat after the agent (or you) goes idle Embody writes any changed TDXN COMP to disk as a frame-cheap `.tdxn` checkpoint (~3-6 ms, no full project save, no strip/restore, no freeze) so an accidental crash loses little unsaved work — the checkpointed COMPs rebuild on next open. Also fires a synchronous pre-checkpoint before a destructive `delete_op` inside a tracked COMP. Bypassed in Perform Mode and during saves, and perf-gated so it never piles onto a hot frame. The two tools that run arbitrary code — `execute_python` and `exec_op_method` — cannot name the COMP they touched, so they arm a *coarse* checkpoint instead: whatever is already queued is written before the code runs, and the settle-drain then discovers which tracked COMPs actually changed, once, after the burst rather than once per call. See [Crash Recovery](externalization.md#crash-recovery).
...
- **What's saved**: Folder path, Envoy config, tag names, tag colors, TDXN settings, logging options, keyboard shortcut bindings (the Shortcuts page, including the tagger double-tap key and the master toggle), and other user-configurable parameters. Read-only status fields and runtime state are excluded
