...
- **There is no paste shortcut — pasting is automatic.** With **Clipboard Auto-Paste** on (default), Embody watches the clipboard and, when a TDN network appears (your copy, or the web "embody it" button), prompts to paste it as a new COMP in the current network. The old ++ctrl+shift+v++ binding was removed: TouchDesigner's native operator-paste fires on the same key, so it pasted stray nodes alongside the TDN. See [Clipboard Auto-Paste](configuration.md).
