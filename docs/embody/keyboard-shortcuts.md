...
- On an **untagged** operator, the tagger lets you pick how to externalize it — a strategy (TOX or TDXN) for a COMP, or a file format for a DAT
...
!!! tip "Keep your externalizations up to date"
    Use ++ctrl+shift+u++ to write all dirty operators to files on disk. On project open, TOX-strategy COMPs are always restored from `.tox` files and DATs sync from their externalized source files (`.py`, `.txt`, `.json`, ...). TDXN-strategy COMPs are reconstructed from `.tdxn` files **only in Roundtrip mode** — in the default Export-on-Save mode the `.toe` stays authoritative and is not rebuilt from `.tdxn` on open. See [TDXN Mode](externalization.md#tdxn-mode-master-switch).

!!! warning "Roundtrip mode strips TDXN containers on Ctrl+S"
    In **Roundtrip mode** (and only when strip-on-save is enabled), saving the `.toe` with ++ctrl+s++ makes Embody **temporarily strip all children** from TDXN-strategy COMPs to keep the `.toe` file small. They are restored immediately after the save completes — but if TD crashes during the save, those children will be missing from the `.toe`. This is fine: they'll be reconstructed from `.tdxn` files the next time the project opens. Just make sure you've updated your externalizations with ++ctrl+shift+u++ first. In the default **Export-on-Save mode**, Ctrl+S does **not** strip: the `.toe` keeps its children and remains the source of truth.

## TDXN Export

- **++ctrl+shift+e++** — Exports the entire project network to a single `.tdxn` file at your project root
- **++ctrl+alt+e++** — Exports only the current COMP's network to a `.tdxn` file
...
!!! info "Export vs Update — what's the difference?"
    ...
    The **Export** shortcuts (++ctrl+shift+e++ / ++ctrl+alt+e++) create a standalone `.tdxn` snapshot of **any** network, whether or not it's externalized. No tracking, no build increment, no side effects. Use these when you want to grab a snapshot of a network you haven't tagged — like exporting someone else's component, creating a one-off backup, or sharing a network as a `.tdxn` file.

## Clipboard (TDXN copy / paste)

- **++ctrl+shift+c++** — Copy the selected COMP to the OS clipboard as a portable TDXN envelope. Paste it into another Embody project, or share it; the web Collection's "embody it" button writes the same envelope.
- **There is no paste shortcut — pasting is automatic.** With **Clipboard Auto-Paste** on (default), Embody watches the clipboard and, when an **inbound** TDXN network appears (e.g. the web "embody it" button), prompts to paste it as a new COMP in the current network. Your own ++ctrl+shift+c++ **outbound** copy is recognized and does **not** trigger the prompt — you copied it to share or paste elsewhere, not to re-import it. The old ++ctrl+shift+v++ binding was removed: TouchDesigner's native operator-paste fires on the same key, so it pasted stray nodes alongside the TDXN. See [Clipboard Auto-Paste](configuration.md).
