...
| Auto-Externalize New Ops (`Autoexternalize`) | Menu | `Neither` | **Opt-in.** Auto-tag operators that Envoy creates via `create_op` for externalization to version-control-friendly files. `Neither` (default) = off -- nothing the AI builds is written to disk unless you tag it yourself. `DATs` = externalize created DATs (source files). `COMPs` = externalize created COMPs (TDN). `DATs and COMPs` = both. Only ops at an externalization boundary are tagged -- never ops already inside an externalized COMP, palette clones, or Embody itself. Additive only: it never deletes files or removes tags. Options: `Neither`, `DATs`, `COMPs`, `DATs and COMPs`. |
