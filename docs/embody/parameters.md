...
| Launch AI Client (`Launchaiclient`) | Pulse | - | Open the AI client selected above at the project root: editors (Cursor, Windsurf; Copilot opens VS Code) open the folder as a workspace; CLI tools (Claude Code, Codex, Gemini) open in a new terminal at the root. |
