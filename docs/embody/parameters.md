...
| Parameter | Type | Default | Description |
|---|---|---|---|
| AI Client (`Aiclient`) | Menu | `0` | Selects the AI client used with Envoy. Determines which MCP configuration file format is generated (e.g. .mcp.json for Claude Code). Options: `Claude Code`, `Codex`, `Gemini`, `VS Code`, `Cursor`, `Windsurf`, `GitHub Copilot`, `None`. |
| Launch AI Client (`Launchaiclient`) | Pulse | - | Open the AI client selected above at the project root: editors (VS Code, Cursor, Windsurf, Copilot) open the folder as a workspace; CLI tools (Claude Code, Codex, Gemini) open in a new terminal at the root. |
