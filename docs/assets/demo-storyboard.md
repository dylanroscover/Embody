...
| 7 | 0:55–1:00 | Cut to a terminal beside the network. `git diff` of the `.tdxn` file — human-readable YAML, additions in green, removals in red. Pause 2 seconds on the diff. | A diff you can read. |
...
- **Pick a `.tdxn` section that diffs cleanly** — not the entire file. Maybe a single annotation group or a single operator's parameter changes. Quality over quantity.
...
- **Stats.** No "52 MCP tools" or "85 test suites." That's README copy. The video is emotional, not technical.
...
- [ ] Terminal pre-staged with large font, in a directory with a clean git state and a `.tdxn` file ready to diff
