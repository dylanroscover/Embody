...
export const specimenGraphs: Record<string, Record<string, unknown>> = {
  ...
  "mandala": {
    ...
    "annotations": [
      {
        "name": "annotate1",
        "title": "Layers (32: one master, 31 clones)",
        "text": "field_disc is the master layer network: circle_line / circle_fill -> LayerId + IsFill attributes -> Nest copies -> Count copies -> out_line / out_fill, plus a Parameter CHOP that emits the Layer + Animate pages as 53 channels -> out_params. Every other layer is a clone of it (Clone + Enable Cloning) that differs only in its parameter values: shape, radius, symmetry, palette slots, nesting, outline style, waves, tumble, colour waves and LFOs. Draw Order stacks layers. To add an element, clone field_disc and wire its three outputs into the merges at the same input index; never copy a layer.",
        "position": [
          -70,
          -6270
        ],
