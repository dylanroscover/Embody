...
| `ADMIN_EMAILS` | optional | Comma-separated allowlist of admin emails that may reach `/admin` (the moderation panel). Always includes the project owner as a bootstrap floor, so the panel works with zero config; promote others from inside the panel via `trust_level='admin'`. See `src/server/admin.ts`. |
