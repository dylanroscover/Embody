...
eight vase treasures, the red palace square with nested colour bands, T-gates and corner
medallions, a dotted lotus seat, two counter-rotating lotus rings, and a rosette, star and
dot at the centre. Every element is a parameterised layer (24 of them) whose outline,
...
- **One shader for every layer.** Each layer is a static chain (circle -> LayerId/IsFill
  ...
  Two GLSL POPs then compute outline, nesting, radial placement, every animation and the
  per-point `Color` / `LineWidth` for all 24 layers at once. Per frame only those two POPs,
...
2. `merge_line` / `merge_fill` (Merge POP, 24 inputs) -> `glsl_line` / `glsl_fill` (GLSL
   POP, `uP` texture buffer from `shuffle_params`, uniforms uTime / uEnergy / uGlobalRot /
   uLoop / uLoopLen / uIsFill) -> `geo_line` (Line MAT, per-point Color and LineWidth) and
   `geo_fill` (Constant MAT with point colour).
...
4. Master pages: **Mandala** (Resolution, Scale, Rotate, Speed Multiplier, Energy, Line
   Width, Loop, Loop Length, Fill Opacity, Time Mode, Manual Time, Saturation, Background),
...
> rate and spin to whole cycles per loop. Compose 24 layers: field disc, four-colour outer
> petal ring, gold rings, lace border, eight vase treasures, bead and tick rings, palace
> square with nested bands and thin nested lines, gates, corner medallions, dotted lotus
